...
- `Given<Subject>` classes with `When<Condition>Then<Outcome>` tests
- arrange/act/assert blocks
- fakes only through intent-named helpers in `Fakes/`
...
{
    // happy flow for every endpoint test
    protected override void ConfigureWebHost(IWebHostBuilder builder) =>
        builder.ConfigureTestServices(services => services.Fake<IProductService>(products => products.HasCatalog()));

    // per-test scenario override
...
public sealed class GivenGetProductsEndpoint(WebApp app)
{
    [Test]
    public async Task WhenCatalogHasProductsThenItReturnsThem()
    {
        using var client = app.Client();

        var products = await client.GetFromJsonAsync<List<ProductDto>>("/api/products");

        await Assert.That(products).IsNotEmpty();
    }

    [Test]
    public async Task WhenCatalogIsUnavailableThenItReturnsServiceUnavailable()
    {
        using var client = app.Client(services => services.Fake<IProductService>(products => products.IsUnavailable()));
...
```csharp
public sealed class GivenEnsureEntityExistsFilter
{
    private static EndpointFilterInvocationContext Invocation(params object?[] arguments) =>
        new DefaultEndpointFilterInvocationContext(new DefaultHttpContext(), arguments);

    [Test]
    public async Task WhenEntityIsMissingThenItReturnsNotFoundWithoutCallingTheHandler()
    {
        var filter = new EnsureEntityExistsFilter<DeleteProduct.Request, Product>(new InMemoryEntityStore<Product>());
        var handlerCalled = false;

        var result = await filter.InvokeAsync(Invocation(new DeleteProduct.Request(42)), _ =>
        {
            handlerCalled = true;
